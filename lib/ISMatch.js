...
(function(){
  ...
  var ISMatch = function(a, b, options){
    ...
    while(ai--){
      ...
      bi = collumns;
      while(bi--){
        var ratio = ISMatch.match(a[ai][0], b[bi][0], options.debug);
        if(ratio != null && (!options.max || ratio < options.max))
          arr[ai][ratio] = bi;
      }
    }
      ...
      while(true){
        ...
        // There is already region x matching the same region y
          ...
          var worst;
            ...
            console.log("TEST: "+ keys[0] +" vs. " + matched[bi][1])
              //debugger

          //if(Math.abs(match_to - a[ix].length) > Math.abs(match_to - a[matched[bi][0]].length)){
          if(keys[0] > matched[bi][1]){
            // obszar b[bi] i a[ix]
          //if(){
    ...
    //console.log(matched)
    return matched.map(function(j,i){return [ b[i][1][0], b[i][1][1], a[j[0]][1][0], a[j[0]][1][1]]})
    ...
    while(i--){
      ...
      while(j--){
        if(i == j){
          ...
          // sinuses
          var cross = (xa-xc)*(yb-yc) - (ya-yc)*(xb-xc);
          var sin = cross / (a*b);

          var acos = Math.acos(ang)
          var x = sin > 0 ? acos : 2*Math.PI-acos
        ...
        var val = (matrix[row+j]/(2*Math.PI))*255;
        d[di] = val;
        d[di+1] = val;
        d[di+2] = val;
        d[di+3] = 255;
