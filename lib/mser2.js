    ...
    var maybeMser = function(component){
      ...
      if(size_function.length > delta){
        ...
        //if(last_speed){
          ...
          if(extremum >= min && size_function[0] == extremum && extremum != last_mser){
            // We've found MSER delta steps ego
            // Find outer boundary of the region
            // Begin with top-left pixel
            var stable_pixels = pixels //.slice(0, -size_function[0]+extremum)
            ...
            var d = [width, 1, -width, -1];
            // Current position
            var position = start + d[0];
            var last_pix;

            // Until full boundary is reached
            while(start != last_pix){
      ...
      return [gray, pixels, size_function, speed, last_mser];
    }
